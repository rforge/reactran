...
\end{enumerate}  
  ...
  The functions from \rt can be use to solve more general types of partial differential equations ($\leq$ order 2).
  
  In this vignette, we give one example of how the package can be used to solve partial differential equations of the parabolic, hyperbolic and elliptic type.
}
...
Usually, the independent variable ``time'' is not discretised, while other variables (e.g. spatial axes) are, so that a (set of) ODE(s) is obtained, which can be solved with appropriate initial values solvers from package \ds \citep{deSolve}.
...
\end{eqnarray*}
Here $D$ is the "diffusion coefficient", $v$ is the "advection rate", and $A_x$ and $\xi$ are the surface area and volume fraction.
...
where \code{C.up} and \code{C.down} are the upstream and downstream boundary values, \code{flux.up} and \code{flux.down} are the respective fluxes, \code{v} and \code{D} are the advection and diffusion coefficient respectively, \code{A} is the surface area, \code{x} contains the grid, and \code{VF} is the volume fraction ($\xi$). 

A suitable grid can be generated with functions \code{setup.grid.1D}, 
\code{setup.grid.2D} (there is no corresponding 3D function), while a property can be added to this grid using functions \code{setup.prop.1D}, and \code{setup.prop.2D}. 
This is useful to have the variable surface areas, volume fractions, advection and diffusion rates being defined at the correct places of the grid.
...
As an example, consider the 1-D diffusion-reaction model, in spherical,
...
with \code{t} the time, \code{r} the (radial) distance from the origin,
\code{Q}, the consumption rate, and with boundary conditions
...
defined; it divides 10 cm (\code{L}) into 1000 boxes (\code{N}).
...
plot (Grid$x.mid, Cartesian$y, type = "l", main = "steady-state PDE",
  lwd = 2, xlab = "x", ylab = "C", col = "red")
lines(Grid$x.mid, Cylindrical$y, lwd = 2, col="blue")
lines(Grid$x.mid, Spherical$y, lwd = 2, col = "green")
...
time units using \ds function \code{ode.1D}:
...
\begin{align*}
   ...
   \label{wavecond}
\end{align*}
where the first condition is a Gaussian pulse.

The implementation in \R starts with defining the box sizes \code{dx}
and the grid, \code{x}.  To comply with the boundary conditions (which
are defined at $\infty$), the grid needs to be taken large enough such
that $u$ remains effectively $0$ at the boundaries, for all times.
...
Note how the initial pulse is split in two pulses, travelling in both
directions, with velocity $c=1$, and maximal value half the original
value. At $t=10$, the two pulses are centered around $-10$ and $+10$.
...
production (\code{p}), consumption
(\code{r * CONC}) and diffusive transport (diffusion coefficients \code{Dx},
\code{Dy}) of a substance (\code{CONC}) in 2 dimensions $(x, y)$; the
