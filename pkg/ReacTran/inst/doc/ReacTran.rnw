...
\end{figure}
  ...
  Note that several properties (\code{D, v, AFDW, VF, A} can be passed
  in different ways:
  \begin{itemize}
...
\section{volumetric advective-diffusive transport in an aquatic system}
  ...
  \begin{verbatim}
    tran.volume.1D(C, C.up = C[1], C.down = C[length(C)],
      ...
      Disp=NULL,   flow = 0, flow.lat=0, AFDW = 1, V=NULL,
      full.check = FALSE, full.output = FALSE)
  \end{verbatim}
  with the following arguments:
  \begin{itemize}
...
tran.2D ( C, C.up=C[1,], C.down=C[nrow(C),],
  ...
  D.x=0, D.y=D.x, v.x=0, v.y=0,
  AFDW.x=1, AFDW.y=AFDW.x, VF.x=1, VF.y=VF.x,
