...
tran.1D <- function(C, C.up=C[1], C.down=C[length(C)],
     flux.up=NA, flux.down=NA, a.bl.up=NULL, C.bl.up=NULL,
...
## INPUT CHECKS
  ...
  if (is.null(v.grid))
    v.grid <- list(int=v)
  if (is.null(VF.grid))
    VF.grid <- list(int=rep(VF,length.out=(length(C)+1)),
                    mid=0.5*(rep(VF,length.out=(length(C)+1))[1:(length(C))]+
                    rep(VF,length.out=(length(C)+1))[2:(length(C)+1)]))
  if (is.null(A.grid))
    A.grid <- list(int=rep(A,length.out=(length(C)+1)),
                   mid=0.5*(rep(A,length.out=(length(C)+1))[1:(length(C))]+
                   rep(A,length.out=(length(C)+1))[2:(length(C)+1)]))
  if (is.null(grid))
    grid <- list(dx=rep(dx,length.out=length(C)),
                 dx.aux=0.5*(c(0,rep(dx,length.out=length(C)))+
                 c(rep(dx,length.out=length(C)),0)))
